...
为已 `git add` 暂存的文件生成 commit，并 push 到当前分支。在 `git commit` 之前必须先执行 `/format-code`，但整个流程仍然不执行 `git add`。
...
2. **在 commit 前执行 `/format-code`**
   ...
   - 如果格式化导致原本 staged 的文件重新变成 modified/unstaged，停止并提示用户先手动 `git add` 这些文件，再重新运行此命令
...
- **格式化后仍不补 stage**：如果 `/format-code` 改动了原本 staged 的文件，也不要替用户重新 `git add`；应停止并提示用户手动复查并重新暂存
